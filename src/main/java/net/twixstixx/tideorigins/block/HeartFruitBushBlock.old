...
import net.twixstixx.tideorigins.TideOriginsMod;
 
 public class HeartFruitBushBlock extends PlantBlock implements Fertilizable {
    ...
    public HeartFruitBushBlock(Settings settings) {
       super(settings);
       this.setDefaultState((BlockState)((BlockState)this.stateManager.getDefaultState()).with(AGE, 0));
    }
 
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
       return new ItemStack(TideOriginsMod.HEARTFRUIT);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
       ...
       if (!bl && player.getStackInHand(hand).isOf(Items.BONE_MEAL)) {
          ...
          dropStack(world, pos, new ItemStack(TideOriginsMod.HEARTFRUIT, j + (bl ? 1 : 0)));
